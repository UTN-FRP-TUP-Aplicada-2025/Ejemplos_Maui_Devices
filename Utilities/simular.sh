...
if [ -z "$UUID" ]; then
    echo "? No se encontr� el simulador iPhone 16 Pro"
...
# Funci�n para timeout en macOS (alternativa a timeout command)
run_with_timeout() {
    local timeout=$1
    shift
...
if [[ "$CURRENT_STATE" != *"Booted"* ]]; then
    ...
    echo "? Simulador ya est� arrancado"
fi
...
# --entitlements inyecta la configuraci�n para que iOS no te bloquee
...
echo "Desinstalando versi�n previa (si existe)..."
...
if xcrun simctl install $UUID "${APP_PATH}"; then
    ...
    echo "Otorgando permisos de notificaci�n..."
    xcrun simctl privacy $UUID grant notifications "${PACKAGE_NAME}" || {
        echo "?? simctl privacy fall� (permisos de macOS). Intentando v�a AppleScript..."
        osascript -e 'tell application "System Events" to tell process "Simulator" to click button "Allow" of window 1' || echo "No se pudo hacer clic autom�tico."
    }
    ...
    echo "? Fall� la instalaci�n"
    exit 1
fi

sleep 10

echo "Verificando instalaci�n...-si no verifica, fue fantasma , copio pero no registro la app"
echo "Verificando si la app es un fantasma..."
if xcrun simctl listapps $UUID | grep -q "${PACKAGE_NAME}"; then
    echo "? Confirmado: La app est� registrada."
else
    echo "? ERROR: La app se instal� pero NO aparece en el sistema (posible problema de firma)."
    exit 1
fi
...
# Forzar creaci�n del contenedor de datos (el Sandbox)
...
echo "Esperando inicializaci�n de la app..."
sleep 5
...
if ls debug_logs/*.txt 1> /dev/null 2>&1; then
    for log in debug_logs/*.txt; do
        ...
        NAME=$(basename "$log")
        if [ "$SIZE" = "102B" ]; then
            echo "  ??  $NAME: $SIZE (posiblemente vac�o)"
        else
            echo "  ? $NAME: $SIZE"
        fi
    done
