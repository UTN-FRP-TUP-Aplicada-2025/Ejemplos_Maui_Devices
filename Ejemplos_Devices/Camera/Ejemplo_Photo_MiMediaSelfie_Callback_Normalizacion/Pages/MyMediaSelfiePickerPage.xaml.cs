...
public partial class MyMediaSelfiePickerPage : ContentPage
{
    ...
    private async Task EvaluarYMostrarEstadoPermisoAsync()
    {
        ...
        {
            MostrarOverlayPermiso("Acceso restringido", "El acceso a la c�mara est� restringido por una pol�tica del dispositivo. Consulte con el administrador.", puedeReintentar: false);
            return;
        }
    ...
    private async Task SeleccionarCamaraAsync()
    {
        ...
        try
        {
            ...
            if (cameraPosition != null)
                MainThread.BeginInvokeOnMainThread(() => _cameraView.SelectedCamera = cameraPosition);
            else
                MostrarOverlayPermiso("C�mara no disponible", "Este dispositivo no tiene cámara disponible.", puedeReintentar: false);
        }
        catch (Exception ex)
        {
