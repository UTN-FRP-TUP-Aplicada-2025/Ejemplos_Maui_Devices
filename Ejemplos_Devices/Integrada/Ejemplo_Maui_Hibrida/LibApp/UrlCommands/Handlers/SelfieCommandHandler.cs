...
public sealed class SelfieCommandHandler : IUrlCommandHandler
{
    ...
    public async Task<BridgeOutcome> HandleAsync(string url)
    {
        ...
        await Shell.Current.GoToAsync(nameof(MyMediaSelfiePickerPage), new ShellNavigationQueryParameters { { "OnPhotoCallback", callback } });
        ...
        byte[]? bytes;
        using (var fs = File.OpenRead(tempPath)) bytes = await _img.ProcesarPhotoAsync(fs);
